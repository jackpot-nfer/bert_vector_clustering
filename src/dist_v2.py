...
class BertEmbeds:
    def __init__(
        ...
        preserve_2g_file,
        glue_words_file,
        bootstrap_entities_file,
    ):
        do_lower = True if do_lower == 1 else False
        self.tokenizer = BertTokenizer.from_pretrained(
            model_path, do_lower_case=do_lower
        )
    ...
    def adaptive_gen_pivot_graphs(self):
        ...
        for key in self.terms_dict:
            ...
            for k in sorted_d:
                ...
                arr.append(k)
            if len(arr) > 1:
                max_mean_term, max_mean, std_dev, s_dict = self.find_pivot_subgraph(
                    arr, tokenize
                )
    ...
    def get_entity_type(self, arr):
        ...
        if len(e_dict) > 1:
            sorted_d = OrderedDict(
                sorted(e_dict.items(), key=lambda kv: kv[1], reverse=True)
            )
            for k in sorted_d:
    ...
    def get_tail_length(self, key, sorted_d):
        rev_sorted_d = OrderedDict(
            sorted(sorted_d.items(), key=lambda kv: kv[0], reverse=True)
        )
        prev_val = 0
        prev_cosine_val = 0
        count = 0
    ...
    def gen_dist_for_vocabs(self):
        ...
        for key in self.terms_dict:
            ...
            if is_rand:
                ...
                if (
                    val < 97
                ):  # this is a biased skip to do a fast cum dist check (3% sample ~ 1000)
                    skip_count += 1
        ...
        final_sorted_d = OrderedDict(
            sorted(cum_dict.items(), key=lambda kv: kv[0], reverse=False)
        )
        ...
        with open("zero_vec_counts.txt", "w", encoding="utf-8") as fp:
            fp.write("Total picked:" + str(picked_count) + "\n")
            final_sorted_d = OrderedDict(
                sorted(zero_dict.items(), key=lambda kv: kv[1], reverse=True)
            )
            try:
                for k in final_sorted_d:
                    # print(k,final_sorted_d[k])
        ...
        with open("tail_counts.txt", "w", encoding="utf-8") as fp:
            fp.write(
                ...
                + str(picked_count)
                + " Average tail len: "
                + str(round(float(total_tail_length) / picked_count, 1))
                + "\n"
            )
            final_sorted_d = OrderedDict(
                sorted(tail_lengths.items(), key=lambda kv: kv[1], reverse=True)
            )
            try:
                for k in final_sorted_d:
                    # print(k,final_sorted_d[k])
    ...
    def calc_inner_prod(self, text1, text2, tokenize):
        if (
            self.cache
            and text1 in self.cosine_cache
            and text2 in self.cosine_cache[text1]
        ):
    ...
    def get_distribution_for_term(self, term1, tokenize):
        ...
        for k in self.terms_dict:
            ...
            if hack_check and val >= 0.6 and val < 0.8 and term1 != term2:
                print(term1, term2)
                str_val = term1 + " " + term2 + "\n"
            ...
            if val >= -0.05 and val <= 0.05:
                zero_dict[term2] = 0
        sorted_d = OrderedDict(
            sorted(dist_dict.items(), key=lambda kv: kv[0], reverse=False)
        )
    ...
    def get_terms_above_threshold(self, term1, threshold, tokenize):
        ...
        sorted_d = OrderedDict(
            sorted(final_dict.items(), key=lambda kv: kv[1], reverse=True)
        )
        return sorted_d
    ...
    def find_pivot_subgraph(self, terms, tokenize):
        ...
        sorted_d = OrderedDict(
            sorted(means_dict.items(), key=lambda kv: kv[1], reverse=True)
        )
    ...
    def calc_bipartite_graph_strength_score(self, terms1, terms2, tokenize, normalize):
        ...
        for i in terms1:
            for j in terms2:
                ...
                full_score += val
        val = (
            float(full_score) / (len(terms1) * len(terms2))
            if normalize
            else float(full_score)
        )
    ...
    def find_entities(self, words):
        ...
        desc_max_term, desc_mean, desc_std_dev, s_dict = self.find_pivot_subgraph(
            words, tokenize
        )
        print(
            "PSG score of input descs:", desc_max_term, desc_mean, desc_std_dev, s_dict
        )
        # OVERRIDE pivot
        ...
        sorted_d = OrderedDict(
            sorted(pivot_similarities.items(), key=lambda kv: kv[1], reverse=True)
        )
        count = 0
        ret_arr = []
        for k in sorted_d:
    ...
    while True:
        ...
        if tokenize:
            ...
            if indexed_tokens == UNK_ID:
                print(
                    "Warning! This is not a token in vocab. Distribution is for UNK token"
                )
...
def graph_test(b_embeds, tokenize):
    while True:
        words = get_words()
        max_mean_term, max_mean, std_dev, s_dict = b_embeds.find_pivot_subgraph(
            words, True
        )
        desc = ""
        for i in s_dict:
...
def bipartite_test(b_embeds, tokenize):
    while True:
        ...
        print(
            "BF score:",
            b_embeds.calc_bipartite_graph_strength_score(words1, words2, True, False),
        )
...
def main():
    if len(sys.argv) != 11:
        print(
            "Usage: <Bert model path - to load tokenizer> do_lower_case[1/0] <vocab file> <vector file> <tokenize text>1/0 <labels_file>  <preserve_1_2_grams_file> < glue words file> <bootstrap entities file>"
        )
    else:
        ...
        b_embeds = BertEmbeds(
            ...
            sys.argv[9],
            sys.argv[10],
        )  # True - for cache embeds; normalize - True
        display_threshold = 0.4
        while True:
            print(
                "Enter test type (0-gen cum dist for vocabs; 1-generate clusters (will take approx 2 hours);  2-neigh/3-pivot graph/4-bipartite/5-Entity test: q to quit"
            )
            val = input()
            if val == "0":
                try:
                ...
                print(
                    "Enter Input threshold .5  works well for both pretraining and fine tuned. Enter 0 for adaptive thresholding"
                )
                val = 0.5
                tail = 10
                try:
