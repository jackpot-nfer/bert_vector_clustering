    ...
    def output_vectors_test(self, sent, is_patched):
        ...
        for vec_i in arr:
            ...
            final_sorted_d = OrderedDict(
                sorted(scores_dict.items(), key=lambda kv: kv[1]["val"], reverse=True)
            )
            count = 0
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print(
            "Usage prog <model path> <vector file extracted from model> <vocab file> <is patched 1/0> <is MLM head vectors 1/0>"
        )
    else:
